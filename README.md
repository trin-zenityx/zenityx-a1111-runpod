...
```text
ghcr.io/trin-zenityx/zenityx-a1111-runpod:0.1.0
```
...
- Extension หลัก: Tag Complete, Ultimate Upscale, Infinite Image Browsing, ControlNet, ADetailer, Batchlinks และ Config Presets
- โปรไฟล์ `colab` สำหรับชุด ControlNet เดิม และ `lite` สำหรับทดสอบเร็ว
...
เพิ่มชุด ControlNet SD1.5 และ IP-Adapter ตาม Colab เดิม รวม asset ที่ประกาศประมาณ 15.0 GiB. นี่คือค่าเริ่มต้นของ image.
...
docker buildx build \
  ...
  -t YOUR_DOCKERHUB_USER/zenityx-a1111:0.1.0 \
  --push .
```
...
- `runpod/template-colab.json` — ชุดใกล้ Colab เดิม ใช้ Volume Disk 50 GB
