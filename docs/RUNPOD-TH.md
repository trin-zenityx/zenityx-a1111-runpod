...
[เปิดหน้า Deploy Full บน RunPod](https://console.runpod.io/deploy?template=f6i7jlc22q)
...
[เปิดหน้า Deploy Lite บน RunPod](https://console.runpod.io/deploy?template=81l0kb0hvr)
...
2. ตรวจชื่อ template:
   - `ZenityX A1111 Full SD15 ControlNet`
   - `ZenityX A1111 Lite`
...
4. รอจนเห็นข้อความ:
   ...
   ```text
   [zenityx] WebUI login: zenityx:รหัสผ่านแบบสุ่ม
   [zenityx] Starting A1111 on port 7860
   ```

5. เก็บ username และ password ไว้ส่วนตัว. รหัสนี้สร้างเฉพาะ workspace และถูก
   เก็บที่ `/workspace/config/gradio-auth.txt` เพื่อใช้ซ้ำเมื่อ restart.
...
4. Login ด้วยข้อมูลจาก Container Logs.
...
หลัง login ให้ตรวจดังนี้:

1. ด้านบนเลือก checkpoint `magmix_v8.safetensors`.
...
| WebUI login | `/workspace/config/gradio-auth.txt` |
...
## 9. เปลี่ยนรหัสผ่านและค่าขั้นสูง

ค่าเริ่มต้นปลอดภัยสำหรับ public template เพราะไม่ได้ฝัง password กลาง. หากต้องการ
กำหนดรหัสเอง ให้สร้าง **RunPod Secret** แล้วอ้าง secret ใน environment variable
`WEBUI_PASSWORD`; อย่าใส่รหัสจริงไว้ใน template ที่จะแชร์.
...
| `WEBUI_USERNAME` | ค่าเริ่มต้น `zenityx` |
| `WEBUI_AUTH` | ต้องเป็น `1` เมื่อเปิดผ่านอินเทอร์เน็ต |
...
### หา WebUI password ไม่เจอ

ค้นคำว่า `WebUI login` ใน Container Logs. หากใช้ workspace เดิม รหัสดูได้จาก
`/workspace/config/gradio-auth.txt` ผ่าน Web Terminal. อย่าโพสต์ค่าจริงใน issue.
...
- URL จาก RunPod proxy เข้าถึงได้จากอินเทอร์เน็ต; ต้องเปิด authentication เสมอ.
